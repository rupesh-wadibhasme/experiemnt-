...
class KerasAnomalyDetectorPyfunc(mlflow.pyfunc.PythonModel):
    ...
    def predict(self, context, model_input: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        if not isinstance(model_input, list):
            item_result = {"status_code": 500, "error": "Input format is not List of dictionaries"}
            log_event(None, log_start, time.time(), None, None, model_input, item_result, "FAILURE", None, time.time(), None, None)
            return [item_result]

        if not model_input:
            item_result = {"status_code": 200, "message": "Send valid input.[non empty]"}
            log_event(None, log_start, time.time(), None, None, model_input, item_result, "OK-EMPTY", None, time.time(), None, None)
            return [item_result]
        ...
        for _, input_dict in enumerate(model_input):
            ...
            try:
                ...
                item_result = {**client_dict, "status_code": 200}
                log_event(
                    ...
                    request_payload=model_input, response_payload=item_result, api_status="OK",
                ...
                item_result = {"status_code": 400, "error": f"Input validation error: {e}"}
                log_event(ID, log_start, time.time(), self.allowed_client_for_this_endpoint, get_model_version(callsign),
                          model_input, item_result, "FAILURE", inference_start_time, time.time(), None, None)
            except KeyError as e:
                item_result = {"status_code": 400, "error": f"Got unexpected key '{e}'"}
                log_event(ID, log_start, time.time(), self.allowed_client_for_this_endpoint, get_model_version(callsign),
                          model_input, item_result, "FAILURE", inference_start_time, time.time(), None, None)
            except Exception as e:
                item_result = {"status_code": 500, "error": f"An unexpected error occurred during processing: {str(e)}"}
                log_event(ID, log_start, time.time(), self.allowed_client_for_this_endpoint, get_model_version(callsign),
                          model_input, item_result, "FAILURE", inference_start_time, time.time(), None, None)
