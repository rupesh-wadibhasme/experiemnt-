...
class KerasAnomalyDetectorPyfunc(mlflow.pyfunc.PythonModel):
    def load_context(self, context):
        ...
        self.unique_BU, self.unique_cpty, self.unique_primarycurr = _get_uniques(
    ...
    def _inference_pipeline(self, data: pd.DataFrame, client_name: str = 'rcc'):
        ...
        current_numeric_columns = _remove_list(current_numeric_columns, ['TDays'])

        cat_data = _one_hot(data[categorical_columns], self.load_scalers['ohe'])
        num_data = _scale(data[current_numeric_columns], self.load_scalers['mms'])
            ...
            try:
                ...
                if isinstance(result, str):
                    ...
                    if df_dev_bu_cpty[df_dev_bu_cpty>cpty_threshold].any().sum()>=1:
                        client_dict['Anomaly'] = 'Yes'
                        llm_input =  llm_util._get_llm_input(llm_util.llm_context,inference_data,df_deviation_all[df_deviation_all>cpty_threshold].dropna(axis=1),fv_tdays_dict)
                        llm_output = llm_util._get_llm_output(llm_util.system_prompt,llm_input)
                        client_dict['Reason'] = llm_output
                    ...
                    if df_deviation[df_deviation>threshold_1].any().sum()>=1:
                        client_dict['Anomaly'] = 'Yes'
                        llm_input =  llm_util._get_llm_input(llm_util.llm_context,inference_data,df_deviation[df_deviation>threshold_1].dropna(axis=1),fv_tdays_dict)
                        llm_output = llm_util._get_llm_output(llm_util.system_prompt,llm_input)
                        client_dict['Reason'] = llm_output
                        #model_details['Inference_time'] = Inference_time
                    elif df_deviation[df_deviation>threshold_2].any().sum()>2:
                        client_dict['Anomaly'] = 'Yes'
                        llm_input =  llm_util._get_llm_input(llm_util.llm_context,inference_data,df_deviation[df_deviation>threshold_2].dropna(axis=1),fv_tdays_dict)
                        llm_output = llm_util._get_llm_output(llm_util.system_prompt,llm_input)
                        client_dict['Reason'] = llm_output
                        #model_details['Inference_time'] = Inference_time
                    else:
