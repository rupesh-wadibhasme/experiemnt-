...
OPTIONAL_COLS = ["BankTransactionId", "IsCashBook"]  # allow IsCashBook as source
...
# ========= Cleaning & header normalization =========
# Header alias map (lowercased, stripped, punctuation-removed key -> canonical)
COLUMN_ALIASES = {
    # canonicals
    "bankaccountcode": "BankAccountCode",
    "businessunitcode": "BusinessUnitCode",
    "banktransactioncode": "BankTransactionCode",
    "valuedatekey": "ValueDateKey",
    "postingdatekey": "PostingDateKey",
    "amountinbankaccountcurrency": "AmountInBankAccountCurrency",
    "banktransactionid": "BankTransactionId",
    "iscurrentday": "IsCurrentDay",
    "ismatched": "IsMatched",
    # cashbook variants
    "cashbookflag": "CashBookFlag",
    "iscashbook": "IsCashBook",
}

def _normalize_headers(df: pd.DataFrame) -> pd.DataFrame:
    """Normalize column headers and map known aliases to canonical names."""
    ren = {}
    for c in df.columns:
        k = str(c).strip().lower().replace(" ", "").replace("-", "").replace("/", "")
        if k in COLUMN_ALIASES:
            ren[c] = COLUMN_ALIASES[k]
...
def _read_excel_selected(path: str, sheet_name=0, drop_dupes: bool = True) -> pd.DataFrame:
    """
    Read ALL columns first, normalize headers, backfill CashBookFlag from IsCashBook if needed,
    and then retain only required/optional columns.
    """
    df = pd.read_excel(
        path,
        sheet_name=sheet_name,
        dtype={DATE_COL_VALUE: str, DATE_COL_POST: str},  # safe if missing
    )
    df = _normalize_headers(df)

    # Backfill CashBookFlag from IsCashBook when CashBookFlag missing
    if FLAG_CASHBOOK not in df.columns and "IsCashBook" in df.columns:
        df[FLAG_CASHBOOK] = df["IsCashBook"]

    # Validate AFTER normalization/backfill
    missing = [c for c in REQUIRED_COLS if c not in df.columns]
    if missing:
        raise KeyError(
            f"Missing required column(s) after header normalization: {missing}\n"
            f"Available columns: {list(df.columns)}\n"
            f"Tip: extend COLUMN_ALIASES if your file uses different header names."
        )

    # Keep only required + optional
    usecols = REQUIRED_COLS + [c for c in OPTIONAL_COLS if c in df.columns]
    df = df[usecols].copy()
...
def engineer_row_level(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # clamp posting lag to avoid outliers due to data errors
    lag = (pst - val).dt.days
...
def fit_featureizer_from_excel(history_xlsx_path: str, sheet_name=0) -> FitArtifacts:
    ...
    if _missing:
        raise KeyError(
            f"Base categorical columns missing in engineered features: {_missing}. "
            f"Available: {list(feats.columns)}\n"
            f"Check header normalization and source file."
        )
    ...
    # ---- final matrix (not returned here, kept for training function) ----
    _ = np.hstack([scaled_part, passthrough_part, X_cat_df.values]).astype("float32")
