...
# Flags ONLY rows where (a) overall recon error is high AND (b) predicted 'amount' != actual (beyond tolerance)
# for the SAME (Account, BU, TxnCode) combo.
...
from bank_features_training_xls import build_training_matrix_from_excel  # <-- adjust if your filename differs
...
def run_pipeline(X_all: np.ndarray, feats_all: pd.DataFrame, feat_names: List[str]):
    ...
    if len(idx_anom) > 0:
        ...
        if (j_feat is not None) and (j_slot is not None) and (scaler_mean is not None) and (scaler_scale is not None):
            ...
            # prefer the raw amount column if present; else use engineered "amount"
