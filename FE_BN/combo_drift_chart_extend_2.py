...
def plot_single_y_norm_drift(
    ...
    # Build global fallback med/iqr for this combo slice (used if stats missing)
    ...
    # Test point y_norm (actual + predicted)
    ts_anom = pd.to_datetime(anomaly_row["ts"])
    combo_str = str(anomaly_row["combo_str"]) if "combo_str" in anomaly_row.index else (
        f"{acct}|{bu}|{code}"
    )

    amt_actual = float(anomaly_row.get("amount", np.nan))
    y_actual = normalize_single_amount(amt_actual, combo_str, stats_train, fallback_med, fallback_iqr)

    has_pred = "amount_pred" in anomaly_row.index
    if has_pred:
        amt_pred = float(anomaly_row["amount_pred"])
        y_pred = normalize_single_amount(amt_pred, combo_str, stats_train, fallback_med, fallback_iqr)
    else:
        y_pred = None
    ...
    # test actual
    ax.scatter(ts_anom, y_actual, marker="o", s=80, edgecolors="black", label="test y_norm (actual)")

    # predicted
    if has_pred and y_pred is not None:
        ax.scatter(ts_anom, y_pred, marker="x", s=90, label="predicted y_norm")
    ...
    ymin = min(y_train.min(), y_actual, (y_pred if y_pred is not None else y_actual), -2.5)
    ymax = max(y_train.max(), y_actual, (y_pred if y_pred is not None else y_actual), 2.5)
    pad = 0.2 * (ymax - ymin + 1e-6)
    ax.set_ylim(ymin - pad, ymax + pad)

    # handle duplicate labels (bands + points); let matplotlib handle it
...
def plot_single_combo_sensitivity(
    ...
    Very simple 'sensitivity' view:
      - For the combo, show absolute amount over time with log-scale option
        or |amount| vs date to see volatility.
    ...
    ts_anom = pd.to_datetime(anomaly_row["ts"])
    amt_abs_anom = abs(float(anomaly_row.get("amount", np.nan)))

    plt.figure(figsize=(9, 4))
    plt.scatter(ts_train, amt_abs_train, alpha=0.6, s=20, label="train |amount|")
    plt.scatter(ts_anom, amt_abs_anom, marker="o", s=80, edgecolors="black", label="test |amount|")

    title = f"Combo Sensitivity: Acct={acct}, BU={bu}, Code={code}"
    plt.title(title)
    plt.xlabel("Date (ts)")
    plt.ylabel("|Amount|")
    plt.yscale("log")  # log scale often makes this chart more informative
