...
def compute_account_baselines(history_df: pd.DataFrame) -> pd.DataFrame:
    """Compute rolling 7D/30D counts/means per account with a 1-step backshift to avoid leakage."""
...
def merge_with_baselines(df: pd.DataFrame, baselines: pd.DataFrame) -> pd.DataFrame:
    """Left-join leakage-free rolling stats to rows (per account) using asof on timestamp."""
