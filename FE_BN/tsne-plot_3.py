...
# ========= NEW: sigma-ellipse helper for cluster boundaries =========
...
def _plot_sigma_ellipses(ax, pts_2d: np.ndarray, n_std_list=(1.0, 2.0)):
    ...
    """
    if pts_2d.shape[0] < 3:
        # not enough points to estimate covariance robustly
        return

    mean = pts_2d.mean(axis=0)
    cov = np.cov(pts_2d.T)  # 2x2
...
def plot_tsne_for_single_combo(
    ...
    # --- NEW: 1σ and 2σ ellipses around train cluster ---
