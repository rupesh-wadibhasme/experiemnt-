...
# - Matches your updated column names
# - Reads Excel
# - Persists OneHot encoder, StandardScaler, and a per-account baseline store (CSV)
# - Per-account asof join to avoid sorting issues
#
# Columns expected:
# ValueDateKey, PostingDateKey, BankTransactionId, BankAccountCode,
# AmountInBankAccountCurrency, BankTransactionCode, MatchedReference,
# IsCurrentDay, BusinessUnitCode, CashBookFlag
...
def engineer_row_level(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # calendar (use posting if available, else value date)
...
def merge_with_baselines(df: pd.DataFrame, baselines: pd.DataFrame) -> pd.DataFrame:
    ...
    # Per-account asof to avoid sort/monotonic issues across accounts
    out_parts = []
    ...
    # Normalize account column names after merge (if suffixes appear)
...
def fit_featureizer_from_excel(history_xlsx_path: str, sheet_name=0) -> FitArtifacts:
    _mkdir(ART_DIR)
    hist = pd.read_excel(history_xlsx_path, sheet_name=sheet_name, dtype={DATE_COL_VALUE: str, DATE_COL_POST: str})

    base = compute_account_baselines(hist)
    base.to_csv(BASELINE_PATH, index=False)  # CSV baseline store
    ...
    enc = OneHotEncoder(handle_unknown="ignore", sparse=False)  # compatible across sklearn versions
...
def transform_excel_batch(batch_xlsx_path: str, sheet_name=0) -> Tuple[np.ndarray, pd.DataFrame, List[str]]:
    ...
    batch = pd.read_excel(batch_xlsx_path, sheet_name=sheet_name, dtype={DATE_COL_VALUE: str, DATE_COL_POST: str})
...
def update_baselines_with_excel(batch_xlsx_path: str, sheet_name=0):
    ...
    new_df = pd.read_excel(batch_xlsx_path, sheet_name=sheet_name, dtype={DATE_COL_VALUE: str, DATE_COL_POST: str})
