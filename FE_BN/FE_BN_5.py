...
# === BEGIN: bolt-on rule helpers (non-business day + daily volume spike) ===
import numpy as np
import pandas as pd
from typing import Set, Tuple

def _to_dt8__safe(s: pd.Series) -> pd.Series:
    """Coerce YYYYMMDD to Timestamp (errors -> NaT); safe if col missing."""
    if s is None:
        return pd.Series(pd.NaT, index=pd.RangeIndex(0))
    return pd.to_datetime(s.astype(str), format="%Y%m%d", errors="coerce")

def _is_weekend__(dt: pd.Series) -> pd.Series:
    return dt.dt.dayofweek.isin([5, 6])

def _is_holiday__(dt: pd.Series, holidays: Set) -> pd.Series:
    if not holidays:
        return pd.Series(False, index=dt.index)
    return dt.dt.date.isin(holidays)

def flag_non_business_day_rules(
    df_raw: pd.DataFrame,
    ...
    """
    Purely rule-based flags:
      - is_nonbiz_value, is_nonbiz_post, nonbiz_reason
    Works directly on your raw/engineered DF without requiring any prior columns.
    """
    holidays = holidays or set()
    out = df_raw.copy()

    vdt = _to_dt8__safe(out[value_col]) if value_col in out.columns else pd.to_datetime(pd.NaT)
    pdt = _to_dt8__safe(out[posting_col]) if posting_col in out.columns else pd.to_datetime(pd.NaT)

    val_weekend = _is_weekend__(vdt)
    post_weekend = _is_weekend__(pdt)
    val_holiday = _is_holiday__(vdt, holidays)
    post_holiday = _is_holiday__(pdt, holidays)

    out["is_nonbiz_value"] = (val_weekend | val_holiday).astype(int)
    out["is_nonbiz_post"]  = (post_weekend | post_holiday).astype(int)

    def _fmt(kind, dtv, why):
        return f"{kind} {dtv.strftime('%Y-%m-%d') if pd.notna(dtv) else 'NA'} is a non-business day ({why})."

    reasons = np.where(
        post_weekend.values,
        [ _fmt("PostingDate", p, "weekend") for p in pdt ],
        np.where(
            post_holiday.values,
            [ _fmt("PostingDate", p, "holiday") for p in pdt ],
            np.where(
                val_weekend.values,
                [ _fmt("ValueDate", v, "weekend") for v in vdt ],
                np.where(
                    val_holiday.values,
                    [ _fmt("ValueDate", v, "holiday") for v in vdt ],
                    [""] * len(out)
                )
            )
        )
...
def flag_daily_volume_spike_rules(
    ...
    """
    Rule-based spike on daily counts for (acct,bu,code, posting_date)
    Adds: group_count_today, hist_mean_30d, hist_std_30d, hist_pctl_30d,
          hist_active_days, vol_spike_flag, vol_spike_reason
    """
    out = df_raw.copy()
    if posting_col not in out.columns:
        raise ValueError(f"Expected '{posting_col}' in dataframe")

    # derive posting date as real date
    pdt = _to_dt8__safe(out[posting_col])
    out["_post_date"] = pdt.dt.date

    # compute daily counts for keys + date
    grp_cols = [*keys, "_post_date"]
    daily = (out.groupby(grp_cols, dropna=False).size().reset_index(name="group_count_today"))

    # rolling stats per group
    def _roll(g):
        g = g.sort_values("_post_date")
        full_idx = pd.date_range(g["_post_date"].min(), g["_post_date"].max(), freq="D")
        tmp = g.set_index("_post_date").reindex(full_idx).rename_axis("_post_date")
        tmp["group_count_today"] = tmp["group_count_today"].fillna(0)

        roll = tmp["group_count_today"].rolling(f"{horizon_days}D")
        mean_ = roll.mean().shift(1)
        std_  = roll.std(ddof=1).shift(1)
        def q(x):
            return np.quantile(x, pct) if len(x) else np.nan
        pctl = roll.apply(q, raw=False).shift(1)
        active = roll.apply(lambda x: int(np.sum(np.array(x) > 0)), raw=False).shift(1)

        rr = (pd.DataFrame({
            "hist_mean_30d": mean_.values,
            "hist_std_30d":  std_.values,
            "hist_pctl_30d": pctl.values,
            "hist_active_days": active.values,
        }, index=full_idx).reset_index().rename(columns={"index":"_post_date"}))
        rr = rr.merge(tmp.reset_index()[["_post_date","group_count_today"]], on="_post_date", how="left")
        rr = rr[rr["_post_date"].isin(g["_post_date"])]
        return rr

    rolled = (daily.groupby(list(keys), group_keys=False)
                   .apply(_roll)
                   .reset_index())

    stats = daily.merge(rolled, on=[*keys, "_post_date"], how="left")

    has_hist = stats["hist_active_days"].fillna(0) >= min_history_days
    if method == "zscore":
        std0 = (stats["hist_std_30d"].fillna(0) == 0) & (stats["group_count_today"] > stats["hist_mean_30d"].fillna(0))
        stdk = (stats["hist_std_30d"].fillna(0) > 0) & (
            stats["group_count_today"] > stats["hist_mean_30d"].fillna(0) + z_k * stats["hist_std_30d"].fillna(0)
        )
        vol = has_hist & (std0 | stdk)
        thresh_txt = np.where(
            std0,
            (stats["hist_mean_30d"].fillna(0)).round(2).astype(str) + " (std≈0)",
            (stats["hist_mean_30d"].fillna(0) + z_k * stats["hist_std_30d"].fillna(0)).round(2).astype(str)
        )
    elif method == "percentile":
        pthr = stats["hist_pctl_30d"].fillna(stats["hist_mean_30d"].fillna(0))
        vol = has_hist & (stats["group_count_today"] > pthr)
        thresh_txt = pthr.round(2).astype(str)
    else:
        raise ValueError("method must be 'zscore' or 'percentile'")

    stats["vol_spike_flag"] = vol.astype(int)
    stats["vol_spike_reason"] = [
        (
            "Txn volume spike: {cnt} today vs typical {typ} for "
            "Account='{acct}', BU='{bu}', Code='{code}' (last {h}d)."
        ).format(
            cnt=int(c), typ=t, acct=a, bu=b, code=cd, h=horizon_days
        )
        for c, t, a, b, cd in zip(
            stats["group_count_today"], thresh_txt, stats[keys[0]], stats[keys[1]], stats[keys[2]]
        )
    ]

    # attach back to row level
    out = out.merge(
        stats[[*keys, "_post_date", "group_count_today", "hist_mean_30d", "hist_std_30d",
               "hist_pctl_30d", "hist_active_days", "vol_spike_flag", "vol_spike_reason"]],
        on=[*keys, "_post_date"],
        how="left"
    )
    # clean up
    out.drop(columns=["_post_date"], inplace=True)

    for c in ["group_count_today","hist_mean_30d","hist_std_30d","hist_pctl_30d","hist_active_days"]:
        if c in out.columns:
            out[c] = out[c].fillna(0)
    out["vol_spike_flag"] = out["vol_spike_flag"].fillna(0).astype(int)
    out["vol_spike_reason"] = out["vol_spike_reason"].fillna("")
    return out

# === END: bolt-on rule helpers ===
